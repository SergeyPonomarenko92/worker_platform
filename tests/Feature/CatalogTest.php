...
class CatalogTest extends TestCase
{
    ...
    public function test_catalog_provider_filter_can_accept_full_provider_url_and_extract_slug(): void
    {
        $cat = Category::factory()->create();

        $bp = BusinessProfile::factory()->create(['slug' => 'demo-provider', 'city' => 'Київ', 'is_active' => true]);
        $other = BusinessProfile::factory()->create(['slug' => 'other-provider', 'city' => 'Київ', 'is_active' => true]);
        ...
        Offer::factory()->for($other)->create([
            'category_id' => $cat->id,
            'title' => 'Other provider offer',
            'is_active' => true,
            'price_from' => 100,
        ...
        $this
            ->get('/catalog?provider=%20https%3A%2F%2Fexample.test%2Fproviders%2Fdemo-provider%20')
            ->assertOk()
            ->assertSessionHasNoErrors()
            ->assertInertia(fn ($page) => $page
                ->component('Catalog/Index')
    ...
    public function test_catalog_provider_filter_trims_trailing_slashes_from_slug_input(): void
    {
        $cat = Category::factory()->create();
        ...
        $this
            ->get('/catalog?provider=demo-provider%2F')
            ->assertOk()
            ->assertSessionHasNoErrors()
            ->assertInertia(fn ($page) => $page
                ->component('Catalog/Index')
